...
class LorentzNet(nn.Module):
    ...
    def __init__(self, hidden_dim: int=48, n_layers: int=4, n_classes: int=5, c_weight: float=0.001, dropout: float=0.1, add_beams: bool=True, beam_mass: float=1.0, normalization: str='batch') -> None:
        super().__init__()
        ...
        self.blocks = nn.ModuleList([LGEB(hidden_dim, n_scalar=self.n_scalar, c_weight=c_weight, normalization=normalization, last_layer=index == n_layers - 1) for index in range(n_layers)])
        self.decode = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden_dim, n_classes))

    def _prepare(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        batch = x.shape[0]
        if self.add_beams:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x, scalars, node_mask = self._prepare(x)
