...
from models.apspool import ApsPool
...
def resnet18_mnist(filter_size=4, pool_only=False, apspool_criterion='l2', N_in=None, **kwargs):
    """Constructs a ResNet-18 model for MNIST.
    Args:
        filter_size (int): Antialiasing filter size
        pool_only (bool): [True] don't antialias the first downsampling operation (which is costly to antialias)
    """
    print('Loading Resnet18-aps model for MNIST')

    # Adjust the first convolution layer to handle 1 channel input for MNIST
    model = ResNet(BasicBlock, [2, 2, 2, 2], filter_size=filter_size, pool_only=pool_only,
                   apspool_criterion=apspool_criterion, N_in=N_in, **kwargs)

    # Modify the first convolution layer for MNIST
    model.conv1 = conv3x3(1, 64, stride=1)

    # Modify the fully connected layer for 10 classes in MNIST
    model.fc = nn.Linear(512 * BasicBlock.expansion, 10)
